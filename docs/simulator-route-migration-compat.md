...
> 🔴 **2026-08-01 §1(공유 링크 대응) 폐기 — 구현하지 마라.**
> 사용자가 *"share 를 메인에서 안 해도 돼. simulator 를 그냥 붙여. 어차피 사용하는 사람이 없었어"* 로
> 결정했다(옛 `/?share=` 링크의 **실사용자 없음을 사용자가 확인**). `/` 는 조건 없이 랜딩을 그리고,
> §1-C 층 1 의 `RootRoute` 래퍼와 `resolveShareRedirectPath`, §1-E 의 `test/router/shareLinkRedirect.test.tsx`
> 는 코드베이스에서 **제거됐다**.
> - **대가**: 옛 `/?share=…`·`/?s=…` 로 들어오면 시나리오가 열리지 않고 랜딩이 뜬다(OG 미리보기 카드는
>   `middleware.ts` 가 `/` 를 계속 매칭해 유지된다).
> - **살아 있는 것**: §1-C **층 2**(middleware `matcher: ['/', '/simulator']`)와 og:url 경로 상수화는
>   그대로 유효하다. `?share=zz` 같은 깨진 페이로드의 **값-기반 실패 처리 + `ShareLinkFailureNotice`**
>   (§1-C 의 🔴 문단)도 `/simulator` 에서 그대로 살아 있다 — 되돌리지 마라.
> - **되살리려면**: `router/routes.tsx` 의 `'/'` 라우트 주석에 사유와 복구 절차가 있다.
> - **§0 과 §2~§11 은 여전히 유효**하다(P0~P2 실행 근거가 여기 있다).
...
## §1 공유 링크 대응 ~~🔴 최우선~~ — ⛔ **2026-08-01 폐기(상단 참고). 아래는 기록용**
