 ...
 *
 * ## 자리
 * 오른쪽 아래다. 두 소비처(커뮤니티 상세 · 랜딩) 모두 이 화면에서 `position: fixed` 인 것은
 * 스킵 링크(왼쪽 위, 포커스 전에는 화면 밖)와 복사 폴백 토스트(`ShareToast` — 위쪽 가운데)뿐이라
 * 아래 구석은 비어 있다(1280·390 실측). 오른쪽을 고른 이유는 스킵 링크가 왼쪽 위에서 내려오고,
 * 이 앱의 주요 액션(공유·좋아요·댓글 등록·랜딩 CTA)이 전부 카드 안 흐름에 있어 겹칠 대상이 없기 때문이다.
 *
