 ...
 * lead    — hero 와 **같은 크기·같은 자리**의 값이되, 시그니처(좌측 오로라 리본 · 액센트 테두리 ·
 *           hero 서체)는 갖지 않는다. 한 줄에 선 지표들이 **서로 대등할 때** 쓴다.
 *           🔴 2026-08-06 국민연금 요약 4칸에서 나왔다: "신고 총액처럼 나머지도 값이 크게 보이고
 *           설명이 그 아래 붙게 통일해 달라"(사용자). 넷을 전부 hero 로 올리면 리본이 넷이 되어
 *           시그니처가 죽으므로, **크기만 hero 급**인 단계를 따로 만들었다.
 * default — 일반 지표. 값이 18px 이고 우측 정렬이라 격자에서 자릿수가 맞는다.
 */
export type StatEmphasis = 'hero' | 'lead' | 'default';
