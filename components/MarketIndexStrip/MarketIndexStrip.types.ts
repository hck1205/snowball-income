...
export type MarketIndexRow = {
  ...
  /**
   * 스크린리더에만 낭독되는 단위. 지수는 ' 포인트', 원/달러는 ' 원' 처럼 **항목마다 다르다**
   * (레지스트리가 정한다). 화면 표시는 어느 쪽도 숫자뿐이다.
   */
  unit: string;
};

/*
 * 🔴 **표시 변형(variant)을 다시 만들지 마라.** 2026-08-02 에 헤더용 축소형을 만들었다가 되돌렸고,
 * 사용자 결정으로 **헤더 배치는 최종 기각**됐다(근거는 `MarketIndexStrip.tsx` 상단 주석의 실측).
 * 이 부품은 본문에 놓는 한 가지 형태뿐이라 **프롭이 없다** — 그래서 Props 타입도 없다.
 */
