...
/**
 * 세션 시작 시 감지된 **디바이스↔클라우드 진짜 동시편집**을 사용자가 3택(이 기기/클라우드/둘 다 합치기)으로
 * 화해하는 모달. 뒤엔 이미 이 기기 상태가 보이고, 여기선 어느 쪽으로 맞출지만 고른다.
 *
 * ⚠ merge-base 덕분에 이 모달은 **양쪽 다 base에서 바뀐 진짜 충돌일 때만, 세션당 1회** 뜬다(단방향 변경은
 *   엔진이 조용히 fast-forward). 그래서 카피·톤을 **차분하게** 둔다 — 경보가 아니라 "어떻게 이어갈지" 선택이다.
 * - 표시 정보는 **경량**이다 — 각 측 탭 개수·이름 목록·마지막 편집 시각(상대표기)만. 무거운 시뮬 계산 없음.
 * - **닫기(Esc/바깥클릭)=이연**: `onDefer`. 무음 화해 금지 — 앱은 이 기기 상태를 유지하고 헤더가 표면화한다.
 * - **둘 다 합치기(블렌드)** 는 비파괴라 기본 권장 + 기본 포커스. 나머지 두 선택은 담담한 보조 설명만 단다.
 *
