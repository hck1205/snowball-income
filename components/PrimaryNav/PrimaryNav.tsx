...
export const COMMUNITY_GROUP_ITEMS = [
  ...
  /*
   * 파이어족들(2026-08-09) — 파이어를 이룬 사람들의 생활을 담은 **유튜브 영상**을 모은다.
   * 🔴 보기는 누구나, **쓰기는 운영자만**이다. 그 제한은 화면이 아니라 DB 가 건다
   *    (마이그레이션 20260810000001) — 그래서 이 항목을 `isAdmin` 조건부로 둘 이유가 없다.
   *    조건부로 두면 nav 가 라우트마다 달라진다(`profileAtom` 은 커뮤니티 안에서만 채워진다).
   * ⚠ 아이콘은 불꽃(Flame): 이름이 '파이어'라 그림이 이름을 그대로 말하고, 이 묶음의 다른 둘
   *   (LayoutGrid·MessageSquare)과 뜻이 겹치지 않는다.
   */
