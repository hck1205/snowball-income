...
export const NavSlot = styled.div`
  ...
  /*
   * 🔴 **좁은 폭에서는 숨는다**(2026-08-07 사용자 지시). 그 폭의 진입점은 브랜드 왼쪽의
   * 드로어 버튼(components/NavDrawer)이다. 종전에는 여덟 항목이 이 줄 안에서 가로 스크롤로
   * 흡수됐는데, 가로로 숨은 항목은 사용자에게 아무 신호도 주지 않는다(pitfalls 2026-07-31 실측).
   * ⚠ 넓은 폭에서는 반대로 드로어 버튼이 숨는다 — 같은 목적지로 가는 길이 둘이면 어느 쪽이
   *   정본인지 알 수 없어진다.
   * ⚠ 경계는 **위 BrandHippo 가 40px 로 줄어드는 폭과 같은 값**이다(2026-08-07 사용자 지시).
   *   그 지점이 헤더가 한 줄짜리 모바일 헤더로 성격을 바꾸는 자리다 — 둘을 따로 두면 어중간한
   *   폭에서 큰 로고 옆에 서랍 버튼이 서는 구간이 생긴다. 한쪽을 바꾸면 다른 쪽도 바꿔라.
   */
...
const logoSlotStyle = `
  ...
  /*
   * 🔴 좁은 폭에서는 **위에 붙는다**(2026-08-07). 그 폭에서 이 영역은 로고와 메뉴 드로어 버튼이
   * 세로로 나눠 쓰는 칸이 된다(버튼은 align-self: end). 로고가 가운데에 그대로 있으면 40px 짜리
   * 둘이 겹친다 — 영역 높이가 두 개를 세우기엔 빠듯하다.
   */
  ${media.down('mobileWide')} {
    align-self: start;
  }
  ...
  /*
   * 🔴 **아주 좁은 폭에서는 그림을 뺀다**(2026-08-09 사용자 결정).
   *
   * 그 폭에서는 헤더에 [☰][그림][Hungry Hippo][로그인·컨트롤] 을 다 세울 자리가 없다 —
   * 실측 320px 에서 워드마크가 로그인 버튼 밑으로 **46px 파고들었다**(360px 은 6px).
   * 경계와 근거 수치는 'BREAKPOINT.mobileNarrow' 주석에 있다.
   *
   * **왜 하필 이걸 빼는가**: 이 그림은 'aria-hidden' 장식이고 이름은 옆 워드마크가 진다.
   * 접근성으로도, 홈으로 가는 길로도 잃는 것이 0 이다 — 워드마크가 이미 홈 링크다.
   * 반대로 워드마크를 줄이면 320px 에서 113 → 67px(59%)이 되어 읽기 어렵고, 잘라 쓰면
   * ("Hungry Hip…") 브랜드가 망가진다.
   * ⚠ 왼쪽이 비어 보이지 않는다 — 같은 폭에서 서랍 버튼(☰)이 그 자리를 지키고 있다.
   *
   * ⚠ 2026-08-03 "하마는 Hungry Hippo 왼쪽에만" 지시와 충돌하지 않는다. 그 지시는 로고가 서는
   *   **자리**를 하나로 모으라는 것이었고(19곳에 흩어져 있던 것을 걷어냈다), 여기서는 자리를
   *   옮기는 것이 아니라 **들어가지 않는 폭에서만 생략**한다.
   */
