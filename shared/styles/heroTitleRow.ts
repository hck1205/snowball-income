 ...
 *
 * 📜 구 히어로 3벌(공용 `components/common/PageHero` + `pages/Portfolio` · `pages/DividendCalendar`
 * 로컬 복제)이 **제목 크기와 아이콘 정렬만은** 갈리지 않게 하려고 만든 파일이다. 히어로는
 * 2026-07-31 에 공용 한 벌로 수렴했지만, 이 두 유틸의 소비처는 히어로 밖에도 있어(카드 제목·
 * 아이콘+문단) 그대로 정본으로 남는다.
 */
...
/**
 * **고르는 카드(`PickCard`) 제목 전용** 크기. 하한 16px(`lg`) · 상한 20px(`2xl`).
 *
 * 왜 `sectionTitleFontSize`(16~18)를 쓰지 않는가: 고르는 카드의 제목은 그 카드에서 **읽히는 유일한
 * 이름**이고(나머지는 보조 설명·수치), 격자 안에서 스캔 대상이 된다. 카드 제목이 차트 카드 헤더와
 * 같은 크기면 격자가 "제목 없는 회색 덩어리"로 읽힌다. 굵기로는 위계를 못 만든다 — `font.display`
 * 는 Bold 한 벌만 실려 600/700/800 이 같게 렌더된다(`tokens.ts` font.display 주석). **크기와 색면뿐이다.**
 *
 * 검산: `0.8rem = 12.8px` 기준 `12.8 + 0.009W = 16` → W = **355.5px**(히어로 제목의 하한 전환점과 같다),
 * `12.8 + 0.009 × 800 = 20.0` → 상한 전환점 **800px**. 히어로와 같은 지점에서 꺾이므로 한 화면에
 * 히어로와 카드 격자가 함께 있어도 두 제목이 어긋난 속도로 줄지 않는다.
 *
 * `vw` 단독을 피하고 `rem` 을 섞는 이유는 위 히어로 주석과 같다(WCAG 1.4.4).
 */
export const pickTitleFontSize = `clamp(${font.size.lg}, calc(0.8rem + 0.9vw), ${font.size['2xl']})`;
