/**
 * **랜딩(`/`) 전용 `FAQPage` JSON-LD 를 다른 라우트의 DOM 에서 떼어 낸다.**
 *
 ...
 *
 * ⚠ **노드를 버리지 않고 보관했다가 되돌려 붙인다.** JSON 본문은 `index.html` 에만 있다(정본 1곳).
 * 여기서 다시 만들면 문자열이 두 벌이 되어 FAQ 카피 개정이 조용히 갈라진다. 그리고 SPA 는
 * `/simulator → /` 처럼 되돌아오므로 제거가 편도면 랜딩이 자기 마크업을 잃는다.
 *
...
/** FAQ 마크업이 유효한 유일한 경로. */
export const FAQ_STRUCTURED_DATA_PATHNAME = '/';
