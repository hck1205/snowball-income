import type { PersistedScenarioState, ScenarioPayload } from './types';

/**
 * 시나리오 페이로드 변환/검증 — **순수 함수**. IO 없음, 테스트 대상.
 *
 * 서버 CHECK 제약(public.is_valid_scenario_payload)과 **같은 규칙**을 클라이언트에도 둔다.
...
export const SCENARIO_PAYLOAD_MAX_BYTES = 65_536;
export const SCENARIO_MAX_TICKERS = 50;
export const SCENARIO_TITLE_MAX_LENGTH = 80;
export const SCENARIO_DESCRIPTION_MAX_LENGTH = 500;
export const COMMENT_BODY_MAX_LENGTH = 2_000;

/**
 * 앱의 시나리오 → 서버 페이로드.
 *
 * 로컬 탭 id/name은 **버린다**: 서버에선 scenarios.title이 제목의 정본이고,
 * 로컬 id를 넣어봐야 다른 사용자에게 의미가 없다 (게다가 페이로드 크기만 먹는다).
 */
export const toScenarioPayload = (scenario: PersistedScenarioState): ScenarioPayload => ({
 ...
 */
export const fromScenarioPayload = (
 ...
 */
export const getScenarioPayloadByteSize = (payload: ScenarioPayload): number =>
  new TextEncoder().encode(JSON.stringify(payload)).length;

export type ScenarioPayloadIssue =
  | 'payload-too-large'
  | 'too-many-tickers'
  | 'missing-portfolio'
 ...
 */
export const validateScenarioPayload = (payload: ScenarioPayload): ScenarioPayloadIssue[] => {
  const issues: ScenarioPayloadIssue[] = [];
  ...
  if (Array.isArray(tickers) && tickers.length > SCENARIO_MAX_TICKERS) {
    issues.push('too-many-tickers');
  }

  if (getScenarioPayloadByteSize(payload) > SCENARIO_PAYLOAD_MAX_BYTES) {
...
export const isScenarioPayloadPublishable = (payload: ScenarioPayload): boolean =>
  validateScenarioPayload(payload).length === 0;
