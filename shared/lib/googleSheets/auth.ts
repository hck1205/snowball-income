 ...
 * ## 무음 갱신 (2026-08-09)
 *
 * 종전 주석은 "무음 갱신은 없다 — `prompt: ''` 도 팝업을 열 수 있어 제스처가 필요하다"였다.
 * **반만 맞다.** `prompt: ''` 는 **이미 동의했고 구글 세션이 살아 있으면 팝업 없이 조용히** 토큰을
 * 준다. 팝업을 여는 것은 그게 **안 될 때**이고, 그때는 제스처가 없어 브라우저가 막으며
 * `error_callback` 으로 떨어진다 — 즉 "언제나 제스처가 필요"가 아니라 "실패하면 제스처가 필요"다.
 *
 * 그래서 `silent: true` 를 받는다. 저장된 연결을 되살릴 때만 쓰고, 실패는 **조용히 삼킨다** —
 * 🔴 사용자가 아무것도 안 눌렀는데 "팝업이 막혔습니다"가 뜨면 그건 거짓 경보다.
 *
 * ⚠ 여전히 **사용자가 시작하는 요청은 클릭 핸들러 안**이어야 한다(팝업이 열릴 수 있다).
 *
...
export const requestAccessToken = async (options?: {
  ...
  /**
   * **무음 시도**인가(사용자 제스처 없이 부른다).
   *
   * 🔴 지금은 호출부에 주는 **신호**일 뿐 동작을 바꾸지 않는다 — `prompt: ''` 는 이미 기본값이고,
   *    조용히 될 상황이면 조용히 되기 때문이다. 이 값이 있는 이유는 **실패를 어떻게 다룰지**가
   *    다르기 때문이다: 무음 실패는 배너를 띄우지 않고 그냥 버튼 화면으로 남는다.
   * ⚠ 그래서 이 함수는 무음이어도 같은 오류를 돌려준다. 삼키는 판단은 호출부가 한다.
   */
