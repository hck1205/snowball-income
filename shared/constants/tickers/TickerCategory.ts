...
export const TICKER_CATEGORY_LABEL = {
  ...
  /**
   * 섹터 ETF(2026-08-26). 코어 지수와 가르는 기준은 **분산의 범위**다 — 코어는 시장 전체를 사는
   * 도구이고, 섹터는 한 산업에만 건다. 둘을 같은 칸에 두면 코어 묶음의 `caution`("배당률이 낮다")이
   * 이쪽에서는 절반만 맞는 말이 된다. 여기서 먼저 말해야 할 것은 배당률이 아니라 **업종 집중 위험**이다.
   *
   * ⚠ 테마 ETF 와 섞지 마라 — 섹터는 표준 산업 분류를 따르고 테마는 이야기를 따른다
   * (`shared/constants/presets/sectorEquityEtfs.ts` 머리말이 그 경계를 이미 정해 두었다).
   */
  sector: '섹터 ETF',
  /**
...
export const TICKER_CATEGORY_META: Record<TickerCategoryId, TickerCategoryMeta> = {
  ...
  sector: {
    metaTitle: '섹터 ETF 모음 — 한 산업에 무게를 더 싣는 자리',
    description:
      '표준 산업 분류를 기준으로 한 업종만 담는 ETF를 모았습니다. 같은 섹터를 건 상품이라도 어디까지를 후보로 삼는지에 따라 구성이 크게 달라지는 지점을 종목별로 짚었습니다.',
    caution:
      '섹터 ETF는 분산이 한 산업 안에서만 이뤄집니다. 그 업종이 조정받으면 시장 전체를 담는 코어 지수보다 깊게 빠지며, 이미 코어 지수를 들고 있다면 같은 종목을 두 번 사는 것이기도 합니다.'
  },
  leveraged: {
  ...
  'growth-stock': {
    ...
    description:
      '배당이 없거나, 배당을 시작한 지 얼마 되지 않았거나, 지급하던 배당을 멈춘 대형 종목을 모았습니다. 배당 이력이 어디까지 쌓였고 어디서 끊겼는지와, 배당이 없는 종목이 이 시뮬레이터에서 어떻게 계산되는지 정리했습니다.',
    caution:
