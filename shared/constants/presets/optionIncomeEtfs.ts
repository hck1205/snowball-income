 ...
 *
 * #### 🔴 2026-08-18: ULTY·JPO 를 넣었다가 **다시 뺐다** — 위 두 줄이 옳았다는 확인
 *
 * 사용자 요청 목록에 둘이 있어 추가했는데, 위 "주 배당"(ULTY)과 "단일 종목형"(JPO — YieldMax 의 JPM
 * 상품) 항목이 **이미 제외해 둔 종목이었다.** 되살리면서 그 두 이유 말고도 **한계 둘을 더 만났다**:
 *
 *  ① **스냅샷 스키마가 배당률 30% 초과를 거부한다**(`MARKET_DATA_BOUNDS.dividendYield.max`).
 *     갱신 시도에서 두 종목이 정확히 그 사유로 rejected 됐다 — 즉 이 계열은 프리셋에 넣어도
 *     **자동 갱신이 배당률을 영원히 덮지 못하고** 손으로 적은 값이 굳는다.
 *  ② 상한 30 을 그대로 쓰면 파생 성장률이 **-23.5%/년** 이 되는데, 그 감쇠를 엔진의 정합 모델이 60년
 *     구간에서 표현하지 못한다(`coherentModel.test.ts` 의 "모든 프리셋이 60년 뒤에도 초기 배당률을
 *     유지한다" 가 YoP 0.3 → 0.0003 으로 무너졌다). 이 파일이 버티는 하한은 SPYT 의 **-13.97** 이고
 *     한계는 그 사이에 있다 — 즉 **분배율 34% 이상은 이 모델로 담을 수 없다**(ETR 하한 6.5 기준).
 *
 * ⇒ 규칙: 배당률이 30%를 넘는 초고분배 상품은 프리셋에 넣지 않는다. 넣으려면 스키마 상한과 엔진의
 *   감쇠 한계를 **함께** 손대야 하고, 그 테스트는 사용자 자산이 걸린 계산 정합성 가드다.
 *
