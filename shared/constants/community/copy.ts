...
export const COMMUNITY_COPY = {
  nav: {
    ...
    brand: 'Hungry Hippo',
    /**
      * 커뮤니티 갤러리 링크(/community/portfolio).
      *
      * 🔴 2026-08-08 사용자 지시로 '포트폴리오 갤러리' → '배당계산 갤러리'. 이 지면에 올라오는 것은
      *    보유 종목 목록이 아니라 **시뮬레이션 결과**라, 새 이름이 내용에 더 가깝다.
      * ⚠ 경로(`/community/portfolio`)와 DB 의 `kind='portfolio'` 는 **그대로다** — 사용자 데이터와
      *   이미 발행된 링크가 그 값을 담고 있다. 바뀐 것은 사람이 읽는 이름뿐이다.
      */
    gallery: '배당계산 갤러리',
    /** 자유게시판 링크(/community/board). */
    board: '게시판',
    /**
     * 파이어족들(/community/firenow) — 커뮤니티 묶음 안, 게시판 다음 자리.
     *
     * 🔴 이 지면은 **유튜브 영상만** 모은다(2026-08-09 사용자 결정). 파이어족의 **생활**을 담은
     *    영상이고, 증시·투자 기사는 게시판의 '뉴스' 분류가 받는다 — 둘은 다른 축이다.
     * ⚠ 이름에 '재테크'·'수익' 같은 말을 넣지 마라. 이 지면이 모으는 것은 돈 버는 법이 아니라
     *   그렇게 사는 사람들의 이야기다.
     */
    fire: '파이어족들',
     ...
     * ⚠ 단독 '포트폴리오' 금지 — 같은 묶음에 `portfolioGroup: '포트폴리오'` 가 있어 서로 헷갈린다.
     ...
     */
    portfolioGroup: '외부 포트폴리오',
    /**
     * 개인 묶음 메뉴 이름 — 자식은 시뮬레이터 · 나의 배당 포트폴리오 · 가계부.
     *
     * 셋의 공통점은 **주어가 나**라는 것이다: 앞으로의 계획(시뮬레이터) · 지금 보유(포트폴리오) ·
     * 들고 나는 돈(가계부). 그래서 `portfolioGroup`("외부 포트폴리오" — 남의 공시 자료)과
     * **내 것 / 남의 것**으로 짝이 된다.
     * 🔴 **묶음이지 목적지가 아니다** — `/my` 같은 라우트는 없다.
     * ⚠ 시뮬레이터는 이 묶음의 **첫 자식**이어야 한다(앱의 핵심 도구다 — 순서를 바꾸지 마라).
     *
     * 🔴 2026-08-14: 이 이름을 '계산하기' 같은 **동사로 바꾸려다 되돌렸다.** 동사 라벨은 첫인상엔
     *    좋지만 "가계부 어디 있지"처럼 아는 것을 찾을 때 짐작을 막는다 — nav 가 하는 일은 **열기
     *    전에** 어디를 열지 알려주는 것이다. 여정은 **칸 순서**에만 담는다(아래 `tickerGroup` 참고).
     */
    personalGroup: '내 자산계획',
    /**
     * 종목 묶음 메뉴 이름 — 자식은 ETF 소개 + 종목 비교.
     * 둘은 원래도 "종목 정보"라는 한 축이라 이웃해 있었다.
     * ⚠ `/ticker/category/:categoryId` 는 **인덱스 라우트가 없다** — 카테고리 허브 역할은
     *   `/ticker/all`(ETF 소개)이 겸한다. 여기에 카테고리 링크를 따로 만들지 마라.
     * 🔴 **묶음이지 목적지가 아니다.**
     */
    tickerGroup: '종목 탐색',
    /**
     * 시장 묶음 메뉴 이름(2026-08-09 신설) — 지금 자식은 시장 온도 하나이고 곧 늘어난다.
     * 🔴 **묶음이지 목적지가 아니다.**
     */
    marketGroup: '시장 읽기',
    /**
     * 시장 온도(/market/pulse) — VIX·신용 스프레드·금리차·심리 지표를 한 화면에 모은다.
     * ⚠ '시장 전망'·'시황' 같은 말을 쓰지 마라 — 이 화면은 관측값을 보여줄 뿐 앞을 말하지 않는다.
     *   이름이 그 선을 먼저 지켜야 한다.
     */
    marketPulse: '시장 온도',
    /**
     * 히포 통계(/market/stats) — 시장 온도의 지표를 겹쳐 보고, 공시된 거래를 종목별로 센다.
     * ⚠ '분석'·'전망' 같은 말을 쓰지 마라. 이 화면이 하는 일은 **세고 옮기는 것**까지다.
     */
    hippoStats: '히포 통계',
    /**
    ...
    /**
     * 미 하원 의원 주식 거래(/portfolio/congress). STOCK Act 정기거래보고서 기반.
     * ⚠ '보유'가 아니라 '거래'다 — 신고서가 사고판 기록이라 보유 현황은 알 수 없다.
     *   이름에서 그 구분이 무너지면 화면 전체가 거짓이 된다.
     * 🔴 2026-08-05 에 '미국'을 이름 앞에 붙였다 — 같은 묶음에 한국 화면이 생겨서,
     *   나라를 말하지 않으면 두 항목이 같은 것으로 읽힌다.
     */
    congressTrades: '미국 의원 주식 거래',
    /**
     * 대한민국 국회의원 주식 보유(/portfolio/korea-assembly). 국회공보 정기재산변동신고 기반.
     * ⚠ '거래'가 아니라 '보유'다 — 연 1회 공개되는 보유 현황이라 사고판 시점을 알 수 없다.
     *   위 `congressTrades` 와 **낱말이 반대**인 것이 핵심이다. 둘을 같은 말로 부르지 마라.
     */
    koreaAssemblyStocks: '한국 의원 주식 보유',
    /**
    ...
    calendarGroup: '캘린더',
    /**
     * 커뮤니티 묶음 메뉴 이름(2026-08-05 신설) — 자식은 배당계산 갤러리와 게시판 둘이다.
     * 🔴 **묶음이지 목적지가 아니다** — `/community` 단독 라우트로 보내지 않는다.
     * 왜 묶었나: 포트폴리오 묶음이 다섯으로 불면서 그 안의 갤러리만 성격이 달랐다(나머지는 공시
     * 자료, 갤러리는 사용자 글). "사람이 쓴 것"을 한 칸에 모은다.
     */
    communityGroup: '커뮤니티',
     ...
     */
    /* 🔴 2026-08-05 사용자 지시로 '배당 리스트' → '배당 종목'. "리스트"는 화면의 **형식**을
       말할 뿐이고, 사용자가 찾는 것은 형식이 아니라 **종목**이다. 되돌리지 마라. */
    dividendListGroup: '배당 종목',
  ...
  board: {
    ...
    /*
     * 🔴 2026-08-05 에 '커뮤니티 게시판' → **'투자 이야기'**(사용자 지시). nav 에 '커뮤니티' 묶음이
     * 생기면서 상위 메뉴와 화면 제목이 같은 낱말을 두 번 말하게 됐다 — 사용자에게는 같은 것이
     * 두 겹으로 보인다. 새 이름은 아래 subtitle 의 나열(질문·고민·인사이트·건의)을 한 낱말로 묶는다.
     * ⚠ nav 항목 이름은 '게시판' 그대로다 — 묶음 안에서 갤러리와 나란히 설 때는 형식을 말하는 편이 짧다.
     */
    mainLabel: '투자 이야기',
    title: '투자 이야기',
  ...
  /**
   * 파이어족들(`/community/firenow`) — **유튜브 영상으로 가는 링크가 본체**인 목록.
   *
   * 🔴 카피가 지는 책임 하나: 이 화면은 **남의 영상을 모으는 곳**이지 우리가 만든 것이 아니라는
   * 사실을 사용자가 알아야 한다. 그래서 제목·리드·빈상태가 "영상"·"원본"이라는 낱말을 쓴다.
   * ⚠ 영상 제목을 우리가 고쳐 쓰지 않는다 — 유튜브가 준 제목 그대로다(`api/unfurl` 의 oEmbed).
   *   대신 **소개 한 줄**(`description`)은 우리가 쓴다. 그게 "왜 이 영상을 골랐는지"를 말한다.
   * ⚠ 작성은 운영자만이다. 그건 화면이 아니라 **DB 가 막는다**(마이그레이션 20260810000001) —
   *   그래서 이 카피에는 "잠시 닫혀 있습니다" 류의 안내가 없다. 보기는 누구나 열려 있다.
   */
  fire: {
    mainLabel: '파이어족들',
    title: '파이어족들',
    subtitle: '파이어를 이룬 사람들의 생활을 담은 영상을 모읍니다. 카드를 누르면 유튜브로 갑니다.',
    write: '영상 추가',
    loadingMore: '더 불러오는 중…',
    reachedEnd: '마지막 영상입니다',
    emptyTitle: '아직 올라온 영상이 없습니다',
    emptySubtitle: '곧 채워집니다.',
    emptyCta: '영상 추가',
    errorTitle: '목록을 불러오지 못했습니다',
    errorBody: '잠시 후 다시 시도해주세요.',
    retry: '다시 시도',
    /** 상세 화면에서 원본으로 나가는 버튼. 새 창으로 열린다는 사실을 낱말이 진다. */
    openOriginal: '유튜브에서 보기',
    /** 썸네일 위 재생 버튼의 접근명 — 그림에는 이름이 없으므로 이 낱말이 그 일을 진다. */
    playHere: '여기서 재생',
    /**
     * 🔴 재생 **전에만** 뜨는 안내. 지우지 마라 — 이 화면이 켜지는 순간 유튜브와 연결된다는
     *    사실을 사용자가 누르기 전에 알아야 한다(개인정보처리방침과 한 벌이다).
     */
    playNotice: '재생을 누르면 YouTube에 연결되며, 그때 YouTube의 쿠키가 설정될 수 있습니다.',
    /**
     * 목록 카드의 표시. 🔴 카드는 유튜브가 아니라 **우리 상세**로 간다(2026-08-09) —
     * 댓글·좋아요가 이 지면의 목적인데 밖으로 먼저 내보내면 거기 닿을 길이 없다.
     */
    openDetail: '자세히 보기',

    /* ── 영상 추가 화면 (운영자 전용) ───────────────────────────────── */
    composeTitle: '영상 추가',
    composeLead: '유튜브 주소를 붙여 넣으면 제목과 썸네일을 자동으로 가져옵니다.',
    urlLabel: '유튜브 주소',
    urlPlaceholder: 'https://www.youtube.com/watch?v=…',
    fetchAction: '미리 보기 가져오기',
    fetching: '가져오는 중…',
    /** 🔴 실패는 조용하면 안 된다 — 무엇을 해야 하는지까지 말한다. */
    fetchFailed: '이 주소에서 영상 정보를 읽지 못했습니다. 주소를 다시 확인해 주세요.',
    /** ⚠ 유튜브 전용이라는 사실을 실패 문구가 말한다 — 다른 주소를 넣은 사람이 이유를 알아야 한다. */
    invalidUrl: '유튜브 영상 주소만 넣을 수 있습니다. (youtube.com/watch · youtu.be)',
    titleLabel: '제목',
    titlePlaceholder: '영상 제목',
    commentLabel: '소개 한 줄',
    commentPlaceholder: '이 영상을 왜 고르셨는지 한두 줄로 적어 주세요. (선택)',
    submit: '올리기',
    submitting: '올리는 중…',
    submitFailed: '올리지 못했습니다. 잠시 후 다시 시도해 주세요.',
    /**
     * 🔴 저작권 고지. 지우지 마라 — 이 화면이 영상을 복제하는 곳이 아니라는 약속이다.
     *    썸네일도 우리가 저장하지 않고 유튜브 주소를 그대로 가리킨다.
     */
    copyrightNote: '영상 내용을 옮겨 적지 마세요. 제목·썸네일·링크만 저장되며, 카드는 유튜브로 연결됩니다.',

    /* ── 운영자가 아닌 사람이 추가 주소로 들어왔을 때 ─────────────────
     * 🔴 "권한이 없습니다"로 끝내지 않는다. 이 지면은 **읽는 것은 누구나** 열려 있으므로,
     *   갈 수 있는 곳을 함께 말해야 막다른 길이 되지 않는다. */
    adminOnlyTitle: '영상 추가는 운영자만 할 수 있습니다',
    adminOnlySubtitle: '올라온 영상은 누구나 보고, 좋아요와 댓글도 남기실 수 있습니다.',
    adminOnlyAction: '파이어족들 보러 가기'
  },
  write: {
    titleNew: '시나리오 공유',
    titleEdit: '시나리오 수정',
    ...
    categoryLabels: {
      ...
      insight: '인사이트',
      /**
       * 증시·투자 관련 기사 공유(2026-08-09 신설, 마이그레이션 20260810000000).
       * ⚠ 파이어족들 지면(`kind='fire'`)과 **다른 축**이다 — 저기는 유튜브 영상, 여기는 기사다.
       */
      news: '뉴스',
      suggestion: '건의사항',
      notice: '공지'
