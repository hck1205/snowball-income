...
export const COMMUNITY_COPY = {
  ...
  news: {
    ...
    /* ── 임시 비공개 안내 (2026-08-08) ──────────────────────────────────
     * `COMMUNITY_NEWS_PUBLIC = false` 일 때 뉴스 주소로 들어온 일반 사용자가 보는 면.
     * 🔴 "없는 페이지"라고 말하지 않는다 — 실제로는 있고 잠시 닫아 둔 것이라, 없다고 하면
     *   다시 열었을 때 이미 떠난 사람에게 거짓말을 한 셈이 된다. 닫힘·재개·다른 갈 곳을 말한다. */
    hiddenTitle: '미디어 뉴스는 잠시 닫혀 있습니다',
    hiddenSubtitle: '지면을 다듬고 있습니다. 준비가 끝나면 다시 열겠습니다.',
    hiddenAction: '배당계산 갤러리로 가기'
  },
  write: {
    titleNew: '시나리오 공유',
    titleEdit: '시나리오 수정',
