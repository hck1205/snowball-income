...
export const COMMUNITY_COPY = {
  nav: {
    ...
    /**
     * 개인 묶음 메뉴 이름(2026-08-09 사용자 결정) — 자식은 시뮬레이터 · 나의 배당 포트폴리오 · 가계부.
     *
     * 셋의 공통점은 **주어가 나**라는 것이다: 앞으로의 계획(시뮬레이터) · 지금 보유(포트폴리오) ·
     * 들고 나는 돈(가계부). 그래서 바로 옆 `portfolioGroup`("외부 포트폴리오" — 남의 공시 자료)과
     * **내 것 / 남의 것**으로 짝이 된다.
     * 🔴 **묶음이지 목적지가 아니다** — `/my` 같은 라우트는 없다.
     * ⚠ 시뮬레이터가 이 안으로 들어가면서 nav 윗줄에서 단독 칸을 잃었다. 앱의 핵심 도구라
     *   묶음의 **첫 자식**이어야 한다(순서를 바꾸지 마라).
     */
    personalGroup: '내 자산계획',
    /**
     * 종목 묶음 메뉴 이름(2026-08-09 사용자 결정) — 자식은 ETF 소개 + 종목 비교.
     * 둘은 원래도 "종목 정보"라는 한 축이라 이웃해 있었다. 묶어서 nav 한 칸을 돌려받았다.
     * 🔴 **묶음이지 목적지가 아니다.**
     */
    tickerGroup: '종목 탐색',
    /**
