...
/**
 * **회귀(실사용자 신고)**: 로그인 상태에서 충돌 모달을 한 번 화해한 뒤, **탭을 삭제하고 새로고침하면
 * 모달이 또 뜨던** 사고. 원인은 로컬 autosave(120ms)는 삭제를 즉시 반영하지만 클라우드 push는 4초
 * 디바운스라, 새로고침이 디바운스를 앞지르면 클라우드에 삭제가 안 가고 로컬{a,b} vs 클라우드{a,b,c}로
 * 세션 시작 엔진이 거짓 충돌을 냈다("이 기기 삭제"를 "다른 기기 추가"와 구분 못 함).
 *
 * 수정 B의 계약: 로컬 ⊂ 클라우드 + 로컬이 엄격히 더 최근 편집이면 = 같은 기기의 삭제→새로고침 레이스라
 * 삭제를 정본으로 push한다(conflict 없이). **사용자 개입(모달) 없이 다음 세션이 noop으로 수렴**함을 단정한다.
 */
