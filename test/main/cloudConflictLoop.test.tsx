 ...
 * conflict(보호 모달, base 이전 동작 복원). 정상 오프라인 편집(같은 계정)은 owner==현재라 여전히 조용한 FF.
 */
describe('M1 계정전환 보호 — 전역 로컬이 다른 계정 것이면 FF 아니라 conflict', () => {
  const dataA = makePayload([makeScenario('a', 'AAA'), makeScenario('b', 'BBB')]);
  const dataB = makePayload([makeScenario('x', 'XXX'), makeScenario('y', 'YYY')]); // 다른 계정, 무관
  const twoTabs = makePayload([makeScenario('a', 'AAA'), makeScenario('b', 'BBB')]);
  const localPlusC = makePayload([makeScenario('a', 'AAA'), makeScenario('b', 'BBB'), makeScenario('c', 'CCC')]);
  ...
  it('계정전환: 전역 로컬이 다른 계정(owner≠현재) 것이면 FF-push 아니라 conflict — A의 클라우드가 B로 안 덮인다', async () => {
    store.local = dataB; // 전역 로컬 = B가 남긴 것(A와 무관)
    store.localUpdatedAt = 9_000; // 로컬(B)이 시각상 더 최신이어도
    store.cloud = dataA; // A의 클라우드
    store.cloudSavedAt = 1_000;
    writeSyncBase('user-1', serializeMeaningfulPayload(dataA)); // base_A == cloud
    writeLocalOwner('user-other'); // 직전 로컬 소유자 = 다른 계정
    liveWorkspace = dataB; // 하이드레이션이 전역 로컬(B)을 화면에 올린 상태

    const jotaiStore = mount();

    await waitFor(() => expect(jotaiStore.get(cloudConflictAtom)).not.toBeNull()); // FF 아님 → 보호 모달
    expect(jotaiStore.get(cloudSyncStateAtom).status).toBe('conflict'); // push 정지
    expect(store.cloud?.scenarios.map((s) => s.id)).toEqual(['a', 'b']); // A의 클라우드 그대로(B로 안 덮임)
  });

  it('같은 계정 오프라인 확장편집(owner==현재)은 여전히 조용한 FF-push(모달 없음)', async () => {
    store.local = localPlusC; // 같은 계정이 CCC 확장
    store.localUpdatedAt = 1_000;
    store.cloud = twoTabs;
    store.cloudSavedAt = 9_000;
    writeSyncBase('user-1', serializeMeaningfulPayload(twoTabs));
    writeLocalOwner('user-1'); // 현재 계정 소유 → foreign 아님
    liveWorkspace = localPlusC;

    const jotaiStore = mount();

    await waitFor(() => expect(store.cloud?.scenarios.map((s) => s.id)).toEqual(['a', 'b', 'c']));
    expect(jotaiStore.get(cloudConflictAtom)).toBeNull(); // 조용한 FF(모달 없음)
    expect(jotaiStore.get(cloudSyncStateAtom).status).not.toBe('conflict');
  });

  it('owner 미상(undefined — 레거시/첫 사용)은 foreign 아님 → 종전대로 조용한 FF(겁주지 않음)', async () => {
    store.local = localPlusC;
    store.localUpdatedAt = 1_000;
    store.cloud = twoTabs;
    store.cloudSavedAt = 9_000;
    writeSyncBase('user-1', serializeMeaningfulPayload(twoTabs));
    // writeLocalOwner 호출 안 함 → owner undefined
    liveWorkspace = localPlusC;
