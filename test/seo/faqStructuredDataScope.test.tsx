 ...
 *
 * `index.html` 은 SPA 전 라우트가 공유하는 셸이라, 아무 조치도 하지 않으면 이 노드가
 * `/simulator`·`/privacy`·없는 주소 어디서나 렌더 결과에 남는다. 그 화면들에는 해당 FAQ 가 **없다** —
 * 렌더링하는 검증 도구가 "보이지 않는 콘텐츠 마크업"으로 보면 FAQ 리치 결과가 사이트 전체에서
 * 무효화될 수 있다. 처방과 되돌리기 조건은 `shared/lib/seo/faqStructuredData.ts` 주석에 있다.
 *
 * 픽스처를 지어내지 않고 **진짜 `index.html` 의 script 태그를 잘라 붙인다** — id 를 한쪽만 바꾸면
 * (=계약 파기) 여기 준비 단계가 먼저 터진다.
 */
...
const FAQ_SCRIPT_MARKUP = (() => {
  ...
  if (!match) {
    throw new Error(`index.html 에서 #${FAQ_STRUCTURED_DATA_ID} script 를 찾지 못했다 (id 계약 파기).`);
  }
  return match[0];
})();
...
describe('FAQPage JSON-LD 의 라우트 범위', () => {
  it('랜딩(`/`)에서는 그대로 남는다', async () => {
    ...
    // 남아 있는 것이 정말 FAQPage 이고 자기 대상을 `/` 라고 말하는지까지 본다.
  ...
  it('시뮬레이터에서 랜딩으로 돌아오면 다시 붙는다 — 제거는 편도가 아니다', async () => {
    ...
    // 다시 붙은 것은 **같은 노드**다 — 본문을 코드에서 재생성하지 않는다(정본은 index.html 하나).
