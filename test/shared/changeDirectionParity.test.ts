...
const REALISTIC_PAIRS: ReadonlyArray<readonly [number, number]> = [
  ...
  /*
   * 🔴 **실제로 도달한 극단값** — 2026-07-31 한국 증시(독립 출처로 확인: 코스피 사상 최대 상승률,
   * 코스닥 역대 2위). 두 구현 어느 쪽도 큰 변동률을 **깎거나 잘라내지 않는다**는 것을 여기서 못 박는다.
   * "비현실적이면 감춘다"는 상한을 누가 한쪽에만 넣으면 이 두 줄에서 갈린다.
   */
  [6595.45, 5593.56], // 코스피 +17.91%
  [719.76, 644.78], // 코스닥 +11.63%
