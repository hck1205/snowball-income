...
/**
 * **부재 계약**: 시뮬레이터 결과 요약 카드는 목표를 서사로 말하지 않는다.
 *
 ...
 *
 * 🔴 이 파일의 존재 이유는 **부재**다. 여기에 "무엇이 있다"를 늘리지 마라 — 특히 "간략히" 토글은
 *    이제 그리드 위 컨트롤 줄(`ScenarioTabsRow`)의 소관이고 그 계약은
 *    `test/main/toggleDesignContract.test.tsx` §3 이 소유한다. 확인하려고 여기 하네스에 그 컴포넌트를
 *    끼워 넣으면 "요약 카드가 무엇을 안 그리는가"라는 이 파일의 초점이 흐려진다.
 */
...
const renderCard = ({ targetMonthlyDividend = 3_000_000, summary = {} }: RenderOptions = {}) => {
  render(
    <Provider store={createStore()}>
      <ResultSummaryCard
        simulation={buildSimulation(
          buildSummary({ targetMonthDividendReachedYear: undefined, ...summary }),
          buildYearly([2026, 2027, 2028])
        )}
...
describe('시뮬레이터 결과 요약 카드 — 목표 서사/진행률 표면 부재', () => {
  it('목표 미설정(target=0)에서 서사도 CTA도 없다 — 타일만 "미설정"으로 말한다', () => {
    renderCard({ targetMonthlyDividend: 0, summary: { targetMonthDividendReachedYear: 2028 } });
  ...
  it('목표 미도달(target>0, 도달 연도 없음)에서 서사도 진행률도 없다', () => {
    renderCard({ targetMonthlyDividend: 3_000_000, summary: { targetMonthDividendReachedYear: undefined } });
  ...
  it('목표 도달(도달 연도 있음)에서도 축하 서사 없이 연도만 말한다', () => {
    renderCard({ targetMonthlyDividend: 3_000_000, summary: { targetMonthDividendReachedYear: 2028 } });

    expectNoGoalNarrativeSurface();
    expect(screen.getByText('2028년')).toBeInTheDocument();
  });

  it('카드 자체가 사라진 게 아니다 — 요약 지표는 그대로 남는다', () => {
    renderCard();
