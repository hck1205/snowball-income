 ...
 * 놓치면 그것이 곧 스크립트 실행이다.
 */
describe('parseLinkPayload — 통과시키면 안 되는 것', () => {
  it.each([
    ['null', null],
    ['undefined', undefined],
    ...
    expect(parseLinkPayload(value)).toBeNull();
  });
  ...
  it.each([
    ...
    expect(parseLinkPayload({ url, title: '제목' })).toBeNull();
  });
...
describe('parseLinkPayload — 좁혀서 통과시키는 것', () => {
  it('제목이 없으면 호스트로 떨어진다 — 빈 카드를 만들지 않는다', () => {
    const parsed = parseLinkPayload({ url: 'https://www.news.example.com/a' });
  ...
  it('제목의 줄바꿈·연속 공백을 한 칸으로 접는다 — 카드 두 줄 제한이 무너지지 않게', () => {
    const parsed = parseLinkPayload({ url: 'https://news.example.com/a', title: ' 앞  \n 뒤 ' });
    expect(parsed?.title).toBe('앞 뒤');
  });

  it('빈 문자열 필드는 없는 것으로 다룬다', () => {
    const parsed = parseLinkPayload({ url: 'https://news.example.com/a', title: '제목', summary: '   ', image: '' });
...
describe('linkHostLabel', () => {
  it('www 를 떼고 호스트만 보여 준다', () => {
    expect(linkHostLabel({ url: 'https://www.hankyung.com/article/1', title: 't', source: 's' })).toBe('hankyung.com');
  });
});
