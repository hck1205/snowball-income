...
// LoginModal 은 이 배럴에서 isNaverEnabled / NAVER_UNDER_REVIEW / selectOAuthFailureGuidance 만 쓴다
// (CommunityModal·SocialLoginButton 은 supabase 를 import 하지 않으므로 이 전체치환 목이 다른 모듈을
// 오염시키지 않는다). detectInAppBrowser·buildKakaoOpenExternalUrl 은 선제 배너 철회로 더 이상 쓰이지 않아
// 목에서도 뺀다.
vi.mock('@/shared/lib/supabase', () => ({
  ...
  // 실 로직은 oauthFailure.test.ts 가 커버한다. 여기선 실패 배너 렌더 두 분기를 결정적으로 몰기 위한 최소 목.
...
describe('LoginModal — 인앱 브라우저에서도 선제 안내를 띄우지 않는다(오진 배너 회귀 방지)', () => {
  // 카카오 로그인 실패의 진짜 원인은 인앱 컨텍스트 분리가 아니라 profiles avatar_url CHECK 제약이었고
  // DB 마이그레이션으로 해소됐다. 그래서 "모달 열림 즉시 겁주던" 선제 배너(외부열기·링크복사)는 제거됐다.
  // 이 블록은 그 선제 배너가 다시 살아나지 않는지(정상 로그인을 막힌다고 오안내하지 않는지)를 못 박는다.
  const KAKAO_UA =
    'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 KAKAOTALK 10.5.0';

  // navigator.userAgent 는 jsdom 에서 그대로 못 덮으므로 defineProperty 로 심고 afterEach 에서 정리한다.
  ...
  it('카카오톡 인앱 UA 로 모달을 열어도(실패 기록 없이) 선제 안내(role=status)가 뜨지 않는다', () => {
    setUserAgent(KAKAO_UA);
    render(<LoginModal onClose={vi.fn()} onSelectProvider={vi.fn()} />);

    // 인앱 안내 배너(구 role=status)가 없다 — 어떤 status 도 노출되지 않는다.
    expect(screen.queryByRole('status')).toBeNull();
    // 실패 기록이 없으니 실패 배너(role=alert)도 없다.
    expect(screen.queryByRole('alert')).toBeNull();
    // 로그인 버튼은 정상 노출.
    expect(screen.getByRole('button', { name: login.google })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: login.kakao })).toBeInTheDocument();
  ...
  it('인앱 UA 에서도 카카오 버튼을 그대로 눌러 로그인할 수 있다', async () => {
    setUserAgent(KAKAO_UA);
    const onSelectProvider = vi.fn();
    render(<LoginModal onClose={vi.fn()} onSelectProvider={onSelectProvider} />);
