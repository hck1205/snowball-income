...
import {
  fromPostPayload,
  getPostPayloadByteSize,
  isPostPayloadPublishable,
  POST_PAYLOAD_MAX_BYTES,
  toPostPayload,
  validatePostPayload
...
describe('toPostPayload', () => {
  it('로컬 탭 id/name을 버리고 portfolio/investmentSettings만 남긴다', () => {
    const payload = toPostPayload(buildScenario([ticker('a')]));
...
describe('fromPostPayload', () => {
  it('서버 페이로드에 로컬 id/name을 다시 붙여 앱 시나리오로 되돌린다', () => {
    const original = buildScenario([ticker('a')]);
    const payload = toPostPayload(original);

    const restored = fromPostPayload(payload, { id: 'new-tab', name: '가져온 시나리오' });
  ...
  it('왕복(앱 → 서버 → 앱)이 계산에 쓰이는 값을 보존한다', () => {
    ...
    const restored = fromPostPayload(toPostPayload(original), { id: original.id, name: original.name });

    expect(restored).toEqual(original);
  });
});

describe('getPostPayloadByteSize', () => {
  it('문자 수가 아니라 UTF-8 바이트 수를 센다 (한글 1자 = 3바이트)', () => {
    const payload = { portfolio: { name: '한글' }, investmentSettings: {} } as unknown as PostPayload;
    const json = JSON.stringify(payload);

    // JSON 문자열 길이보다 크다 = 멀티바이트를 제대로 반영했다는 뜻
    expect(getPostPayloadByteSize(payload)).toBeGreaterThan(json.length);
  });
});

describe('validatePostPayload', () => {
  it('정상 페이로드는 문제 없음', () => {
    expect(validatePostPayload(toPostPayload(buildScenario([ticker('a')])))).toEqual([]);
    expect(isPostPayloadPublishable(toPostPayload(buildScenario()))).toBe(true);
  });

  it('티커 50개 초과를 잡는다 (서버 CHECK와 동일 규칙)', () => {
    const tickers = Array.from({ length: 51 }, (_, index) => ticker(String(index)));

    expect(validatePostPayload(toPostPayload(buildScenario(tickers)))).toContain('too-many-tickers');
  });

  it('티커 50개는 통과한다 (경계값)', () => {
    const tickers = Array.from({ length: 50 }, (_, index) => ticker(String(index)));

    expect(validatePostPayload(toPostPayload(buildScenario(tickers)))).not.toContain('too-many-tickers');
  });

  it('64KB 초과 페이로드를 잡는다', () => {
    const payload = {
      portfolio: { tickerProfiles: [], padding: 'x'.repeat(POST_PAYLOAD_MAX_BYTES) },
      investmentSettings: {}
    } as unknown as PostPayload;

    expect(validatePostPayload(payload)).toContain('payload-too-large');
    expect(isPostPayloadPublishable(payload)).toBe(false);
  });

  it('필수 키가 없으면 잡는다', () => {
    const payload = {} as unknown as PostPayload;
