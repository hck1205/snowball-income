...
export const HolderStrip = styled.div`
  ...
  /*
   * ⚠ **손가락에는 hover 가 없다.** 그래서 터치에서는 focus-within 이 유일한 펼침 경로다 —
   *   칩을 한 번 누르면 그 순간 포커스가 들어가 무리가 펼쳐진다(누른 칩의 드로어도 함께 열린다).
   *
   * 🔴 종전에는 이 구간에서 **처음부터 펼쳐** 두었는데, 2026-08-07 사용자 지적으로 되돌렸다:
   *   "모바일에서 프로필이 겹치는 효과가 없다". 겹침은 이 화면의 표현이지 마우스 전용 절약이
   *   아니고, 펼쳐 두면 좁은 폭에서 칩이 줄을 여러 개로 밀어 올려 타일 높이도 들쭉날쭉해진다.
   * ⚠ 대신 터치에서는 칩을 **한 단 크게** 잡는다 — 겹친 상태에서 실제로 보이는 폭이 25px 이라
   *   손가락 히트 영역이 얇아진다(44px 권장 대역은 아래 HolderChip 의 세로 높이가 진다).
   */
  @media (hover: none) {
    > button + button {
      margin-left: -13px;
