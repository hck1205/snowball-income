...
/**
 * 랜딩 페이지의 세로 리듬과 히어로 곁 덩어리.
 *
 * 🔴 이 파일은 **면을 하나도 만들지 않는다.** 랜딩의 틴트 면은 정확히 2개이고
 * (히어로 그라디언트 · 시작 준비 카드의 wash) 둘 다 각자의 컴포넌트가 소유한다.
 */

/**
 * **그룹 경계**의 간격 — 이 스택의 직계 자식은 이제 섹션이 아니라 서사 묶음이다
 * (히어로 · G2 배우기 · G3 고르기 · G4 참조와 마무리 · 푸터).
 *
 * 🔴 값이 하나뿐이면 그룹이 표현될 수 없다 — before 는 8곳 전부 같은 간격(실측 1280 50.6px ·
 * 390 32px)이라 8섹션이 균일한 카드밭으로 읽혔다. 이 값은 그룹 **안** 간격(LandingGroup,
 * 24~40px)의 정확히 2배 대역이라 "묶음"이 눈에 보인다(실측 after 1280 76.8 / 38.4).
 */
