...
export const DIVIDEND_CALENDAR_COPY = {
  ...
  picker: {
    ...
    openShort: '종목 선택',
    close: '종목 선택 닫기',
    /* ── 시장 탭(2026-08-07 사용자 지시) ──────────────────────────────────
     * 🔴 한 목록에 두 시장이 섞여 있었다(미국 다수 + 한국 상장 12종). 찾는 쪽은 매번 걸러 읽어야
     *   했고, 한국 종목은 티커가 숫자라 알파벳 사이에 묻혔다. 탭으로 가른다.
     * ⚠ 탭 라벨에 **개수를 함께** 낸다 — 빈 탭을 눌러 보고 나서 알게 하지 않는다. 검색 중이면
     *   그 검색 결과의 개수라, 반대 탭에 몇 개가 걸렸는지도 누르기 전에 보인다.
     */
    marketGroupLabel: '시장',
    marketUs: '미국 주식',
    marketKr: '한국 주식',
  ...
  nav: {
     ...
     * 없애도 되지 않을까?"). 2026-07-26 에 아이콘 전용으로 줄였는데, 달력 그림은 "이번 달로
     * 돌아간다"를 말하지 못한다 — 캘린더 화면 안의 달력 아이콘은 특히 아무것도 가리키지 않는다.
     * 기능은 쓸모가 있으니 지우는 대신 **읽히게** 만든다. 미국 증시 캘린더가 같은 기능을 이미
     * "이번 달" 텍스트 버튼으로 내고 있어, 두 캘린더의 문법도 이 김에 맞는다.
     */
    today: '이번 달',
  ...
  /**
   * 화면 최상단 **요약 데크**의 문구(2026-08-03 리워크).
   *
   * 이 화면의 질문은 "이번 달 언제 들어오나"다. 예전에는 그 답이 달력 42칸을 다 훑은 뒤에야
   * 나왔다(1280 실측: 달력 보드가 문서의 45%, 실제 일정 목록은 스크롤 아래). 데크는 그 답을
   * **가장 먼저** 말하는 자리다 — 다음 예상 지급일과 남은 날짜.
   *
   * 🔴 **금액은 말하지 않는다.** 이 화면의 데이터에는 지급'액'이 없다(지급월·예상 지급일뿐).
   * "얼마"를 적고 싶어지는 순간이 이 화면 최대의 날조 지점이다.
   */
  deck: {
    nextLabel: '다음 예상 지급',
    /** 표시 중인 달의 예상 지급이 전부 지났을 때. */
    pastLabel: '이 달 마지막 예상 지급',
    /**
     * 남은 날. 0 을 'D-0' 으로 적지 않는다 — 숫자만으로는 오늘인지 읽히지 않는다.
     * ⚠ '오늘' 단독 문자열을 만들지 마라: 달력 칸의 sr-only '오늘'과 같은 말이 되어
     *   "화면에 '오늘'은 정확히 하나"라는 계약이 깨진다.
     */
    countdown: (days: number) => (days === 0 ? '오늘 지급 예정' : `D-${days}`),
    countdownPast: '지난 일정',
    dateLine: (month: number, day: number, weekday: string) => `${month}월 ${day}일 (${weekday})`,
    /** 칩으로 다 못 보여 준 종목 수. */
    tickerMore: (n: number) => `외 ${n}종`,
    /**
     * 데크에서 그 날짜 일정으로 가는 버튼의 접근명.
     * ⚠ 달력 칸의 이동 버튼(`board.dayJump`)과 **다른 문장**이어야 한다 — 같으면 한 화면에
     *   같은 이름의 버튼이 둘이 되어 무엇을 누르는지 소리로 구분되지 않는다.
     */
    jumpToDay: (month: number, day: number) => `${month}월 ${day}일 일정 보기`,
    /** 선택이 0종일 때 데크가 대신 하는 말. */
    emptyLabel: '선택한 종목 없음',
    emptyLead: '관심 종목을 고르면 다음 예상 지급일과 남은 날짜가 이 자리에 표시됩니다.',
    /** 선택은 있는데 이 달에 놓인 지급이 없을 때. */
    noneLabel: '이 달 예상 지급 없음',
    noneLead: '달을 넘겨 다음 지급이 있는 달을 확인하실 수 있습니다.'
  },
  /** 좌측 주역 열(지급 일정)의 이름. 안쪽 목록의 region 이름('지급 일정 목록')과 겹치지 않게 짧다. */
  ledger: {
    sectionLabel: '지급 일정',
    /**
     * 제목 옆 건수 — "몇 건짜리 목록인가"를 제목과 같은 줄에서 먼저 말한다.
     * ⚠ 데크의 집계 문장(`board.summary`)과 **단위가 같아야 한다**(건). 한쪽만 '종'으로 쓰면
     *   같은 화면에서 같은 수가 다른 단위로 두 번 보인다.
     */
    count: (n: number) => `${n}건`,
    /**
     * 같은 카드 안쪽 두 번째 구역(종목 × 12개월 표)의 이름.
     *
     * 🔴 이 표는 **넓은 쪽 열**에 산다. 성격만 보면 달력과 같은 훑어보기라 우측 개관 열이
     * 맞아 보이지만, 13열짜리 표를 460px 열에 넣으면 데스크톱에서도 가로 스크롤이 남는다 —
     * 성격보다 **표가 요구하는 폭**이 배치를 정한다. 실측(1440): 왼쪽 열에서는 스크롤 0.
     */
    legendLabel: '연간 지급 리듬'
  },
  board: {
    /**
     ...
     */
    sectionLabel: '월간 지급 캘린더',
    caption: (label: string) =>
      `${label} 배당 지급 예정 캘린더. 날짜 칸에 그 날 지급 예정 종목이 표시됩니다. 지급 예정이 있는 날짜는 눌러서 지급 일정 목록으로 이동할 수 있습니다.`,
    ...
    moreCountAria: (n: number) => `외 ${n}종, 지급 일정 목록 참고`,
     ...
     * 🔴 방향어("아래")를 쓰지 않는다: 2026-08-03 리워크로 목록이 지도 **위**(모바일)·
     *   **왼쪽**(데스크톱)에 서면서 "아래"가 어느 폭에서도 참이 아니게 됐다.
     */
    jumpHint: '지급 예정이 있는 날짜를 누르면 지급 일정 목록에서 그 날 일정을 볼 수 있습니다.'
  },
  /**
