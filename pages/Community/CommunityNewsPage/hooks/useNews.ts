...
import { fetchNewsPage, getSupabaseClient, type CommunityClient, type NewsListItem } from '@/shared/lib/supabase';
...
/**
 * 미디어 뉴스 목록 데이터 훅 — `kind='news'` 최신순 + keyset 무한스크롤.
 *
 ...
 *
 * ⚠ 마이그레이션(20260807000000) 전에는 `kind='news'` 행이 존재할 수 없어 **빈 목록**이 온다 —
...
export const useNews = (): UseNewsResult => {
  ...
  const loadFirstPage = useCallback(async () => {
    ...
    try {
      const page = await fetchNewsPage(client, { sort: 'recent', cursor: null });
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const page = await fetchNewsPage(client, { sort: 'recent', cursor });
      if (requestId !== requestIdRef.current) return;
