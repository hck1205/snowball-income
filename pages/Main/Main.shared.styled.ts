...
export const PresetChipScrollArea = styled.div`
  /*
   * 🔴 화면 높이에 따라 조금 늘어난다(2026-08-11). 종전 130px 고정은 이 화면이 **가운데 뜬 520px
   *    모달**이던 시절의 수치였다. 드로어로 옮긴 뒤에는 패널이 길어졌는데 목록만 3줄이라 화면이
   *    비어 보였다(사용자 지적).
   * ⚠ 그렇다고 **길게 빼지 않는다**(2026-08-11 사용자 지시). 이 목록 바로 밑에 담은 목록이 오고,
   *   그 둘이 각자 스크롤을 갖는 구조다 — 한쪽이 화면을 다 먹으면 다른 쪽이 접혀 보이지 않는다.
   *   상한 240px ≈ 칩 5~6줄로, 목록과 담은 목록과 생성 버튼이 한 화면에 함께 선다.
   */
