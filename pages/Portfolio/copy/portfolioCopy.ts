...
export const PORTFOLIO_COPY = {
  ...
  hero: {
    ...
    dDay: {
      /**
       * 🔴 **세 줄이 한 문장이 되게 쓴다**(2026-08-05 사용자 지시: 종전 문구가 너무 사무적이다).
       * 종전에는 "다음 배당 지급 예정일까지 / D-23 / DES" 였다 — 세 줄 모두 정보였고, 마지막 줄은
       * 종목 코드 하나가 덩그러니 놓여 있어 명세서처럼 읽혔다. 지금은 위→아래로 읽으면
       * **"다음 배당까지 · D-23 · DES 배당이 들어올 예정입니다"** 한 문장이 된다.
       *
       * ⚠ 그래도 **단정하지 않는다.** 이 날짜는 과거 지급 이력에서 계산한 추정이라 어미는 계속
       *   "예정"이다(각주 footnote.schedule 과 같은 근거). 따뜻하게 쓰되 없는 확신을 만들지 마라.
       * ⚠ 해요체 금지(`test/shared/copyTone.test.ts`) — 부드럽게 쓰려다 "들어와요"가 되면 잡힌다.
       */
      label: '다음 배당까지',
      /** 오늘이 지급 예정일이면 "며칠 남았다"가 아니다 — 문장 자체를 바꾼다. */
      todayLabel: '기다리던 그날, 오늘입니다',
      value: (days: number) => `D-${days}`,
      todayValue: 'D-DAY',
      /**
       * 세 번째 줄. 종목 코드만 두지 않고 **무슨 일이 일어나는지**까지 말한다.
       * 🔴 종목이 여럿이면 앞의 `tickerSummary` 가 "DES 외 2종"으로 접어 넘긴다 — 이 문장은
       *    그 결과를 받기만 한다(종목 나열 규칙을 두 곳에 두지 않는다).
       */
      tickerLine: (tickers: string) => `${tickers} 배당이 들어올 예정입니다`,
      todayTickerLine: (tickers: string) => `${tickers} 배당이 오늘 들어올 예정입니다`
    }
  },
  ...
  /**
   * 월간 리캡 — **한 해의 배당 리듬과 이번 달의 자리**(2026-08-07, 평가서 P1-⑤).
   *
   * 🔴 여기 숫자는 전부 **예상**이다(연 배당 ÷ 지급월 수). "받았다"·"입금"처럼 확정으로 읽히는
   * 낱말을 쓰지 마라 — 이 앱은 실제 입금 내역을 갖고 있지 않다.
   */
  recap: {
    label: '올해 배당 리듬',
    /** 지난달 대비. 부호는 문장이 지고 색은 거들 뿐이다. */
    change: (percent: number) =>
      `지난달보다 ${percent >= 0 ? '+' : '−'}${Math.abs(percent).toFixed(1)}% 예상됩니다`,
    /**
     * 지난달과 사실상 같을 때. ⚠ "+0.0%" 로 적으면 어색하고 아무것도 말하지 않는다 —
     * 매달 같은 금액이 들어오는 구성(월배당 ETF)에서는 이 문장이 기본값이 된다.
     */
    flat: '지난달과 비슷한 수준입니다',
    /** 지난달 배당이 0일 때. 🔴 퍼센트로 적으면 거짓이 되므로 문장으로 말한다. */
    noComparison: '지난달에는 예정된 배당이 없었습니다',
    rhythm: (count: number) => `열두 달 중 ${count}개 달에 배당이 예정되어 있습니다.`,
    estimateNote:
      '연 배당을 지급 횟수로 나눈 예상치입니다. 실제 금액과 시점은 각 종목의 공시에 따라 달라집니다.'
  },
