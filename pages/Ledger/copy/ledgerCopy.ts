...
export const LEDGER_COPY = {
  ...
  /**
   * B-3 **두 가계부 블렌딩** — 부부·연인이 각자 쓰는 가계부를 한 화면에서 "우리 가계"로 본다.
   *
   * 🔴 **읽기 전용이다**(D3-4). 이 절에는 추가·수정·삭제 문구가 없고, 대신 각 행에서 그 가계부의
   * 단일 뷰로 넘어가는 `openSource` 하나만 있다. 문구를 더할 때 쓰기 동사를 들이지 마라.
   * 🔴 **라벨은 사용자가 붙인 이름**이고 기본값은 데이터 층(`LEDGER_BLEND_DEFAULT_LABEL`)이 소유한다 —
   *    여기에 `'가계부 1'` 같은 사본을 적지 마라(저장된 라벨과 화면 기본값이 갈린다). 이 파일이 그 상수를
   *    재export 하지 않는 이유는 `copy → utils → ledgerFormat → copy` 순환 때문이다(설정 화면의 초기값은
   *    훅이 `labelsOfLedgerBlendConfig` 로 만든다).
   * 🔴 **라벨 뒤에는 조사를 직접 붙이지 않는다.** 사용자가 붙인 이름은 받침을 알 수 없어
   *    `{label}을(를)` 이 되므로, 언제나 `{label} 가계부를` 처럼 **명사를 하나 끼워** 조사를 그쪽에 건다.
   * ⚠ 실패 문구를 `error.*`(저장 실패)에서 빌려 쓰지 않는다 — 이 화면은 읽기만 하므로
   *    "저장하지 못했습니다"는 사실과 다르다. 아래 `reason` 이 읽기 쪽 문장을 따로 갖는다.
   */
  blend: {
    open: '두 가계부 합쳐 보기',
    ...
    exit: '한 가계부만 보기',
    settings: '합칠 가계부 바꾸기',
    /** 🔴 앱이 통화 차이를 감지할 방법이 없다(D3-6) — 그래서 전제를 문장으로 말한다. */
    ...
    setup: {
      ...
      /**
       * 🔴 두 구성 경로를 **모두** 열어 둔다. "같은 시트의 두 탭"이 권장 경로지만(D3-1), 서로 다른
       * 시트 경로가 막혔다고 단정하지 않는다 — 미확인이지 실패가 아니다(스펙 §8).
       */
      ...
      legendB: '두 번째 가계부',
      pick: '가계부',
      /** 이름을 아직 못 읽은 선택지. 🔴 시트 주소를 대신 보여 주지 않는다(준PII). */
      unnamedOption: (index: number) => `저장된 가계부 ${index}`,
      loadingNames: '가계부 이름을 불러오는 중입니다.',
      label: '이 가계부를 부를 이름',
      labelHint: (max: number) => `배지와 소계에 그대로 나옵니다. ${max}자까지 입력할 수 있습니다.`,
    ...
    view: {
      ...
      caption: (label: string) => `${label} 두 가계부를 합친 수입·지출 기록`,
      summaryNet: (label: string) => `${label} 두 가계부 순액`,
      columnSource: '가계부',
      subtotalTitle: '가계부별 소계',
      /** 🔴 합산과 소계를 같은 말로 부르지 않는다 — 소계는 "그 가계부 하나의 숫자"다. */
      subtotalCaption: '두 가계부를 따로 센 수입·지출입니다. 위 합계는 이 둘을 더한 값입니다.',
      loading: '두 가계부를 읽는 중입니다.',
      emptyMonth: (label: string) => `${label}에 두 가계부 모두 기록이 없습니다.`,
      unreadable: (label: string, count: number) =>
        `${label} 가계부에는 형식을 읽지 못한 기록이 ${count}건 있습니다. 이 합계에 포함되지 않았습니다.`,
      /** 🔴 D3-4 — 이 화면에는 쓰기 버튼이 없다. 어디서 고치면 되는지를 대신 말한다. */
      readOnly: '합쳐 보기 화면에서는 기록을 고치지 않습니다. 고칠 기록은 그 가계부를 열어서 바꿉니다.',
      openSource: (label: string) => `${label} 가계부에서 열기`,
      openRowAria: (date: string, category: string, amount: string, label: string) =>
        `${date} ${category} ${amount} 기록을 ${label} 가계부에서 열기`,
      /**
       * "이 가계부에서 열기"가 막혔을 때 사유 줄에 **덧붙이는** 한 문장.
       *
       * 🔴 막혔다는 **판단과 사유 문장**은 탭 전환과 같은 단일 출처(`tabSwitchBlockedReason`)가
       * 소유한다 — 여기서 사유를 다시 쓰지 마라(두 표면의 규칙이 갈린다). 이 문장만 덧붙이는 이유는
       * 그 사유가 가리키는 "아래 목록"(저장하지 못한 기록)이 **합쳐 보기 화면에는 서지 않기**
       * 때문이다. 해결 경로가 화면에 없는 채로 막기만 하면 사용자가 갇힌다.
       */
      openBlockedHint: '한 가계부만 보기로 돌아가면 저장하지 못한 기록을 다시 저장할 수 있습니다.',
      /** 다른 스프레드시트의 가계부는 지금 연결을 갈아끼워야 열 수 있다 — 할 수 있는 척하지 않는다. */
      openElsewhere: '다른 시트에 있는 가계부는 이 화면에서 바로 열 수 없습니다. 시트를 다시 골라 연결하면 볼 수 있습니다.',

      /** 🔴 AC3-5 — 반쪽 데이터로 합계를 내지 않는다. 0 이나 대시로 채우지도 않는다. */
      partialTitle: '두 가계부의 합계를 표시할 수 없습니다',
      partialBody: (label: string) =>
        `${label} 가계부를 불러오지 못해 합계를 계산하지 않았습니다. 아래는 불러온 가계부 하나의 기록입니다.`,
      failureTitle: (label: string) => `${label} 가계부를 불러오지 못했습니다`,
      unavailableTitle: '두 가계부를 모두 불러오지 못했습니다',
      retry: '다시 불러오기',
      /**
       * 읽기 실패 사유. 🔴 값이 섞이지 않는 **정적 문구**다(시트 주소·탭 제목·금액 금지).
       * ⚠ `error.*` 는 저장 실패의 목소리라 여기서 재사용하지 않는다.
       */
      reason: {
