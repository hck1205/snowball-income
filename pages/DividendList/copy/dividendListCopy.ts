...
const LISTS: Record<DividendListId, DividendListPageCopy> = {
   ...
   */
  hiddenStars: {
    metaTitle: '배당 히든스타 — 배당킹·귀족·챔피언에 없는 배당성장 종목',
    metaDescription:
      '배당킹·배당귀족·배당챔피언 어디에도 없지만 배당을 오래 늘려 온 미국 기업 목록입니다. 선정 규칙과 기준일을 함께 표기했습니다.',
    title: '배당 히든스타',
    lede: '세 목록의 문턱을 넘지 못했을 뿐, 배당은 오래 늘려 온 기업들입니다.',
    definition:
      '배당킹·배당귀족·배당챔피언은 모두 연속 증배 연수라는 한 가지 자를 씁니다. 그 자에 몇 해가 모자라면 어느 목록에도 실리지 않습니다. 이 목록은 그 틈에 있는 종목을 섹터를 가리지 않고 모은 것으로, 바깥 기관의 명부가 아니라 아래 규칙을 통과한 종목만 담습니다.',
    criterionLabel: '세 목록에 없음 + 배당률 2.5% 이상 + 5년 배당성장률 4% 이상',
    caution:
      '이 목록은 다른 셋과 달리 공인된 명부가 아니라 이 서비스가 정한 규칙의 결과입니다. 규칙이 바뀌면 구성도 바뀝니다. 배당률이 또래보다 크게 높은 종목은 배당이 늘어서가 아니라 주가가 내려서 그렇게 보일 수 있어, 해당 종목에는 표에서 따로 표시했습니다. 연속 증배 연수는 후보를 모은 ETF가 보장하는 하한이며 정확한 연수가 아닙니다.',
    hub: {
...
export const DIVIDEND_LIST_COPY = {
  ...
  monthlyHighYieldNotice:
    '배당률이 또래보다 크게 높습니다. 배당이 늘어서가 아니라 주가가 내려서 그렇게 보일 수 있습니다.',
