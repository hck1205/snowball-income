...
--
-- 🔴 함수 이름이 DB 마다 다르다 (2026-08-07 첫 실행이 여기서 42883 으로 실패했다)
-- ---------------------------------------------------------------------------
-- 20260714000000 은 이 함수를 `is_valid_scenario_payload` 로 만들었고,
-- 20260723000000(scenarios → posts 개명)이 `is_valid_post_payload` 로 **rename** 했다.
-- 개명을 실행한 DB 와 아닌 DB 가 갈릴 수 있으므로, 이 파일은 이름을 가정하지 않고
-- **실재하는 쪽을 찾아 쓴다**(아래 2단계).
--
-- 왜 컬럼을 더하지 않나
-- ---------------------------------------------------------------------------
...
-- 멱등성 / 실행 순서
-- ---------------------------------------------------------------------------
-- 함수는 create or replace, 제약은 찾아서 drop → add. 두 번 돌려도 결과가 같다.
...
--   ② 시나리오 검사 함수 이름 — 개명 실행 여부에 따라 is_valid_post_payload 또는
--      is_valid_scenario_payload 다.
-- 🔴 함수 찾기가 **드롭보다 앞**이다. 없는데 드롭부터 하면 되돌릴 것 없이 제약만 잃는다.
--   둘 다 없으면 아무것도 바꾸지 않고 예외로 멈춘다 — 조용히 넘어가면 시나리오 payload 검사가
--   통째로 사라진 채 제약이 다시 걸린다(그게 더 나쁘다).
do $$
declare
  target record;
  scenario_fn text;
begin
  ...
  if scenario_fn is null then
    raise exception '시나리오 payload 검사 함수를 찾지 못했습니다. 20260714000000 을 먼저 실행하세요.';
  end if;
