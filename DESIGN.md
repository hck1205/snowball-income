...
- ❌ **"눈덩이 / 스노우볼" 비유 전면 금지 — 예외 없음**(2026-08-03. "브랜드명 suffix는 예외" 조항은
  제품명이 구 이름이던 시절의 것이라 폐기됐다. 제품명은 영문 **Hungry Hippo** 하나이고 한글 음차를 만들지 않는다).
  마스코트·일러스트·카피 어디에도 그 모티프를 쓰지 않는다. 같은 개념은 **복리·시간·재투자**로 푼다.
...
```
```
"눈덩이·스노우볼" 비유를 쓰지 않는다(예외 없음). 숫자·표·입력·결과 카드에는 성격을 넣지 않는다.
