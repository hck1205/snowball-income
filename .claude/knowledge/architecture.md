...
- **[2026-07-23] 실측: Yahoo `chart` API가 KR 상장 ETF도 커버한다** — `query1.finance.yahoo.com/v8/finance/chart/458730.KS`(TIGER 미국배당다우존스, KRX) 직접 호출로 `regularMarketPrice`(현재가) + `events.dividends`(분배금 이력 23건) 정상 반환 확인. 즉 기존 `yahooProvider`에 `.KS`/`.KQ` 접미사만 붙이면 **KR+US 월배당 ETF의 현재가·배당이력을 단일 소스·단일 파이프라인으로** 받는다(브리핑 전제 "한 API로 KR+US 못 준다"가 이 케이스에선 깨짐). ⚠ 한계는 그대로: 비공식·SLA 없음, **정식 지급일(payment_date)은 안 줌**(배당락일 근사만) → 실지급 캘린더는 여전히 SEIBro/공공데이터 딥다이브(ETF 분배금 포함 여부 미확인) 필요. 저가 공식 백업 후보 = EODHD $19.99/mo(003690.KQ 커버 확인).
...
- **콘텐츠 계약 착수(2026-07-22)**: `shared/constants/tickers/`에 `TickerContent`(섹션 배열+FAQ+reference-only 팩트+disclaimer) 타입과 SCHD 1종 엔트리 랜딩. 이 토대(엔진 6필드 확장 데이터)와는 **별개 관심사**로 분리했다 — `TickerReferenceFacts`는 여기 토대B가 다루는 `TickerReferenceEntry`(운용보수·섹터·CAGR 등 구조화 데이터)와 필드가 겹치지만, `TickerContent.reference`는 "서사에 곁들이는 근사치+asOfNote 고지"이고 토대B의 파이프라인 산출물은 "자동 갱신되는 구조화 데이터"다. 파이프라인이 `fetchProfile`/`fetchSchedule`을 실제로 붙이면 `TickerContent.reference`의 정적 필드 상당수를 그 산출물로 대체할 수 있다(지금은 수작업 조사값). 상세는 decisions.md "티커 SEO 콘텐츠" 섹션.
- **크롤러 인프라 착지(2026-07-22, 같은 세션)**: `pages/Ticker/`(TickerHubPage·TickerDetailPage, 라우트 `/ticker/all`·`/ticker/:name`, router/routes.tsx에 lazy 등록 완료 — 병렬 프론트 트랙 산출물)와 `server/handlers/TickerHtml/`(크롤러 HTML, `PostHtml.ts` 미러 — 이 세션 산출물)가 **서로 모르는 채로 독립 작업했는데 라우트 경로(`/ticker/:name`, 허브 `/ticker/all`)가 정확히 일치**했다(둘 다 이번 미션 브리핑의 예시 경로를 그대로 채택). `vite.config.ts`의 `seoAssetsPlugin`도 `TICKER_CONTENT_LIST`를 esbuild 메모리 평가로 읽어 `dist/sitemap-pages.xml`에 파생 등록했다 — 토대B의 "ETF 상세(#5, SEO 라우트+sitemap)"가 Phase 3 계획보다 앞당겨 착수된 상태다. `api/ticker-html.js` 번들은 `DIVIDEND_UNIVERSE`(전체 프리셋 유니버스)를 통째로 끌고 와 ~196KB(post-html.js 82KB 대비 큼) — 서버리스 함수 한도엔 문제없지만 티커 콘텐츠가 수백 개로 늘면 재점검 대상.
