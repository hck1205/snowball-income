...
- 커뮤니티 콜드스타트 **시딩 설계(spec-only, 2026-07-18)**: `docs/community-seeding/`(personas.json 20명·personas.md·README 설계). **아직 구현/실계정/실글 없음** — "일단 페르소나만" 지시. 구현 착수 전 사용자 결정 3건 대기(투명성 라벨 / 제거 임계치 / 시드 규모, README §6). ⚠ 실무 하드 제약 2가지(코드에서 옴): ① 시드 계정은 `profiles.id`가 `auth.users` FK라 **service_role admin으로만** 생성(anon 불가, VITE_ 금지) ② like/comment/view_count가 트리거·generated라 **인기수를 직접 못 박고** 실제 like row가 필요 → 20명이면 글당 최대 ~19 좋아요(과열 위장 불가). 콘텐츠는 §1 윤리 게이트(보장·펌프·실적위조 금지) 통과분만.
